    ...
    {filteredAds?.map((ad: AdProps) => (
      ...
      >
        ...
        <article className="mt-2 px-2 py-2 flex gap-2 justify-between flex-auto">
          ...
          <p className="md:text-sm text-xs leading-snug tracking-normal text-blue-gray-900 antialiased">{truncateText(ad.desc, 20)}</p>
          </div>
          <div className="flex flex-col gap-1">
          <p className="md:text-sm text-xs flex flex-co gap-1 justify-center text-inherit"><Wallet size={20} />
           <span className="font-bold whitespace-nowrap">{ad.price} Kč</span>
          </p>
          <p className="md:text-sm text-xs flex gap-1 justify-center text-inherit"><House size={20}/>
          <span>{truncateText(ad.city, 10)}</span>
          </p>
          <p className="md:text-sm text-xs flex gap-1 justify-center text-inherit"><Layers3 size={20}/>
